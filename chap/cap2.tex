...
\begin{figure}[H]
	...
	\end{center}
	\caption{\label{tunelrasoeprofundo} Distinção entre túnel raso e profundo (adaptado de \citeonline{Benamar1996})}
\end{figure}
...
Há diversos métodos de escavação que podem ser empregados na execução de túneis. Para a escolha do mais adequado, muitos fatores devem ser levados em consideração, como o impacto da construção do túnel nos arredores, as características do solo ou rocha, as dimensões e aspectos do túnel, a segurança e o impacto ambiental \cite{Chapman2018, Moreira2006}. 

Apesar da distinção entre solo macio e solo duro (rocha), as técnicas de escavação de túneis estão sendo aplicadas em uma variedade mais ampla de condições de solo. Enquanto o solo macio precisa de apoio imediato após a criação do vazio, um solo estável permite que a construção do túnel se concentre na economia e seja conduzida pelos limites dos equipamentos a serem utilizados \cite{Chapman2018}. O tempo que o vazio permanece estável sem suporte é chamado de tempo de permanência.

Os tipos de escavação podem ser categorizados em dois grupos principais: não mecanizados e mecanizados. As escavações não mecanizadas englobam técnicas como vala recoberta, escavação simples e detonações, enquanto as mecanizadas incluem \textit{Tunnel Boring Machines} (TBM), as chamadas tuneladoras, e cravação de tubos com macacos hidráulicos \cite{Quevedo2017}.

\colorbox{yellow}{fluxograma?}

Uma escavação do tipo vala recoberta é utilizada em túneis de superfície e pode ser executada de duas maneiras: \textit{cut and cover} e \textit{cover and cut}. A técnica \textit{cut and cover} consiste na escavação de uma trincheira a partir da superfície, sucedida da construção do túnel totalmente independente das paredes de suporte da trincheira, o qual é concluído antes de ser coberto por material de aterro e ter a superfície restaurada. Por outro lado, na técnica \textit{cover and cut}, as paredes de suporte da escavação consistem na estrutura final das paredes do túnel, e a superfície é reintegrada antes da sua conclusão \cite{FederalHighwayAdministration2009}. A \autoref{valarecoberta} apresenta o processo construtivo da escavação do tipo vala recoberta.
